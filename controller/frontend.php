...
function addComment() {
    if(isset($_GET['id_post']) and $_GET['id_post'] > 0) {
        if (isset($_POST['author']) AND isset($_POST['comment']) AND !empty($_POST['author']) AND !empty($_POST['comment'])) {
            $commentManager = new CommentManager;
            ...
            //header('Location: index.php?action=post&id_post='.$_GET['id_post']);
            throw new Exception('Le commentaire n\'a pas été posté. Veuillez renseigner un auteur et un commentaire');
        }
    } else {
